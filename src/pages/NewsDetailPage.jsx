  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const keywords = news.title.split(' ').slice(0, 3).join(' ');
        const q = encodeURIComponent(`${keywords} ${news.category || 'teknoloji'}`);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="px-6 pb-6">
            ...
            <div className="prose prose-lg max-w-none">
              ...
              {articleText && articleText.length > 300 && (
                <div className="mb-6">
                  <button
                  ...
                  >
                    <svg 
                      className={`w-5 h-5 transition-transform ${showFullContent ? 'rotate-180' : ''}`} 
                      fill="none" 
                      stroke="currentColor" 
                      viewBox="0 0 24 24"
                    >
                    ...
                    {showFullContent ? 'Tam İçeriği Gizle' : 'Tam İçeriği Göster (İngilizce)'}
                  </button>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <a 
                        ...
                        className="text-blue-600 hover:text-blue-800 font-medium underline break-all text-sm block"
                        onClick={(e) => {
                          console.log('Kaynak link tıklandı:', news.url);
                        }}
                      >
                        {news.source?.name || (() => { try { return new URL(news.url).hostname; } catch { return 'Kaynak'; } })()}
                      </a>
                      <p className="text-xs text-gray-500 mt-1">Orijinal haberi okumak için tıklayın</p>
                      <p className="text-xs text-gray-400 mt-1 font-mono">{news.url}</p>
                    </div>
                  </div>
                </div>
              )}
