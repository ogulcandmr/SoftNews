...
export default async function handler(req, res) {
  ...
  try {
    if (provider === 'gnews') {
      ...
      const queries = [
        'https://gnews.io/api/v4/search?q=technology OR software OR AI OR hardware&lang=en&max=30&apikey=' + encodeURIComponent(GNEWS_API_KEY),
        'https://gnews.io/api/v4/search?q=teknoloji OR yazılım OR yapay zeka&lang=tr&max=30&apikey=' + encodeURIComponent(GNEWS_API_KEY),
        'https://gnews.io/api/v4/search?q=startup OR innovation OR tech news&lang=en&max=20&apikey=' + encodeURIComponent(GNEWS_API_KEY)
      ];
