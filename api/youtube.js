// Vercel Serverless - YouTube API (Netlify'den tam kopya)
...
export default async function handler(req, res) {
  ...
  const max = Math.max(1, Math.min(50, Number(req.query.max || 9)));
  
  try {
    const data = await fetchYouTube(q, max);
    return res.status(200).json(data);
  } catch (e) {
    return res.status(200).json({ ok: true, source: 'fallback_exception', items: FALLBACK_VIDEOS });
  }
}
